...
def generarMenu():
    with st.sidebar:
        ...
        st.page_link(path + '/Home.py', label='Inicio', icon='🏡')
        st.page_link(path + '/pages/02_reto.py', label='Reto PEBC', icon='💱')
        st.page_link(path + '/pages/03_data_original.py', label='BBDD Original', icon='📅')
        st.page_link(path + '/pages/05_data_def.py', label='BBDD Definitiva', icon='💯')
        st.page_link(path + '/pages/06_mod_reg_lineal.py', label='Modelo de Regresión Lineal', icon='💹')
        st.page_link(path + '/pages/07_analisis_var_usario.py', label='Prueba de Variables y Modelos', icon='🕹️')
        st.page_link(path + '/pages/09_pronostico.py', label='Pronóstico', icon='🎯')
        st.write("---")  # Línea divisoria horizontal
        st.page_link(path + '/pages/10_bootcamp.py', label='BootCamp', icon='⛺')
        st.page_link(path + '/pages/11_creditos.py', label='Créditos', icon='🔗')
        st.page_link(path + '/pages/12_nosotros.py', label='Nosotros', icon='📸')
...
def Logos_y_Titulo(logo1, logo2, logo3):
    ...
    with col6:
        st.image(logo3)
    # Crear el título
    #st.markdown("<p style='color: mediumslateblue;'>PEBC - Precio de la Energía en la Bolsa de Colombia.</p>\n", unsafe_allow_html=True)
...
def SeleccionAutomaticaPaleta(modelo_deseado):
